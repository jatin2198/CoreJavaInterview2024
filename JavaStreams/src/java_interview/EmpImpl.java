...
Employee e2=new Employee("Quora", "Delhi", 24, email, mobile);
...
Map<String, List<Employee>> map=list.stream().collect(Collectors.groupingBy(Employee::getAddress));
	...
	list.stream().sorted(Comparator.comparing(Employee::getName).reversed()).forEach(emp->System.out.println(emp));
	}
